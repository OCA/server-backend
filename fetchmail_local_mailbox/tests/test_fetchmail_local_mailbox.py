...
class TestFetchmailLocalMailbox(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.env["mail.alias.domain"].create(
            {
                "name": "fetchmail_local_mailbox_test",
            }
        )
    ...
    def test_fetchmail_delete(self):
        ...
        self.assertFalse(
            os.listdir(
                os.path.join(
                    self.fetchmail_server.mailbox_path,
                    "cur",
                )
            )
        )

    @mute_logger("odoo.addons.fetchmail_local_mailbox.models.fetchmail_server")
