...
class DavCollectionFieldMapping(models.Model):
    ...
    def _from_vobject_code(self, child):
        ...
        context = {
            ...
            'dateutil': safe_eval_tools.dateutil,
            'item': child,
            'result': None,
            'tools': safe_eval_tools.wrap_module(tools, TOOLS_SAFE_ATTRS),
            'tz': safe_eval_tools.wrap_module(tz, TZ_SAFE_ATTRS),
            'vobject': safe_eval_tools.wrap_module(vobject, VOBJECT_SAFE_ATTRS),
        }
    ...
    def _to_vobject_datetime(self, value):
        ...
        if not result:
            return result
        if result.tzinfo:
            return result.astimezone(tz.UTC)
        return result.replace(tzinfo=tz.UTC)
