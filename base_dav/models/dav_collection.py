...
class DavCollection(models.Model):
    ...
    def get_record(self, components):
        ...
        if self.field_uuid:
            field_name = self.field_uuid.name
            if self.field_uuid.ttype in ("integer", "many2one"):
                try:
                    key = int(key)
    ...
    def dav_list(
        ...
        result = []
        for record in self.eval_domain_records():
            uuid = record[self.field_uuid.name] if self.field_uuid else str(record.id)
            ...
            if self.field_uuid:
                ...
                if self.field_uuid.ttype in ("integer", "many2one"):
                    try:
                        ...
                        clean_key = None
                if clean_key is not None and self.field_uuid.name not in data:
                    data[self.field_uuid.name] = clean_key
